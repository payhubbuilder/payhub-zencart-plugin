...
class payhub{
	      ...
	      $selection = array('id' => $this->code,
	                         ...
	                         'fields' => array(array('title' => MODULE_PAYMENT_PAYHUB_TEXT_CREDIT_CARD_OWNER,
	                                           ...
	                                           array('title' => MODULE_PAYMENT_PAYHUB_TEXT_CREDIT_CARD_NUMBER,
	                                                 'field' => zen_draw_input_field('payhub_cc_number', '', 'id="'.$this->code.'-cc-number"' . $onFocus . " autocomplete=\"off\""),
	                                           ...
	                                           array('title' => MODULE_PAYMENT_PAYHUB_TEXT_CVV,
	                                                 'field' => zen_draw_input_field('payhub_cc_cvv','', 'size="4", maxlength="4"' . ' id="'.$this->code.'-cc-cvv"' . $onFocus . " autocomplete=\"off\"") . ' ' . '<a href="javascript:popupWindow(\'' . zen_href_link(FILENAME_POPUP_CVV_HELP) . '\')">' . MODULE_PAYMENT_PAYHUB_TEXT_POPUP_CVV_LINK . '</a>',
  ...
  function before_process() {
    ...
    $new_order_id = "zencart_" . (string)$new_order_id . '-' . zen_create_random_value(4);

    //This overrides the admin setting. Leave for now since you shouldn't be able to change it anyways.
    ...
    $data = array(
		...
		"CUSTOMER_WEB" => "",
		"CUSTOMER_PHONE_NUMBER" => preg_replace('/[^0-9]/', '', $order->customer['telephone']),
		"CUSTOMER_PHONE_EXT" => "",
		"CUSTOMER_PHONE_TYPE" => "",
  ...
  function install() {
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('API Username', 'MODULE_PAYMENT_PAYHUB_USERNAME', 'api_username', 'This your API Username', '6', '0', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('API Password', 'MODULE_PAYMENT_PAYHUB_PASSWORD', 'api_password', 'This is your API Password', '6', '0', now())");
